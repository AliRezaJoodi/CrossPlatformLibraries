...
//*************************************************
static inline void AJ_QEI_A_ConfigAsInput(aj_qei_t *qei){
    AJ_BitReg_ClearBit_Mask(qei->chA.ddr, qei->chA.mask);
}

static inline void AJ_QEI_A_ConfigAsPullNone(aj_qei_t *qei){
    AJ_BitReg_ClearBit_Mask(qei->chA.port, qei->chA.mask);
}

static inline void AJ_QEI_A_ConfigAsPullUp(aj_qei_t *qei){
    AJ_BitReg_SetBit_Mask(qei->chA.port, qei->chA.mask);
}

//*************************************************
static inline void AJ_QEI_B_ConfigAsInput(aj_qei_t *qei){
    AJ_BitReg_ClearBit_Mask(qei->chB.ddr, qei->chB.mask);
}

static inline void AJ_QEI_B_ConfigAsPullNone(aj_qei_t *qei){
    AJ_BitReg_ClearBit_Mask(qei->chB.port, qei->chB.mask);
}

static inline void AJ_QEI_B_ConfigAsPullUp(aj_qei_t *qei){
    AJ_BitReg_SetBit_Mask(qei->chB.port, qei->chB.mask);
}

//*************************************************
#if (AJ_QEI_AB_CLOSED == 0U)
    static inline uint8_t AJ_QEI_A_Read(aj_qei_t *qei){
        return AJ_BitReg_IsBitSet_Mask(qei->chA.pin, qei->chA.mask);
    }

    static inline uint8_t AJ_QEI_B_Read(aj_qei_t *qei){
        return AJ_BitReg_IsBitSet_Mask(qei->chB.pin, qei->chB.mask);
    }
#else
    static inline uint8_t AJ_QEI_AB_Read(aj_qei_t *qei){
...
//*************************************************
#if (AJ_QEI_Z_USED == 0U)
static inline void AJ_QEI_Z_ConfigAsInput(aj_qei_t *qei){
    AJ_BitReg_ClearBit_Mask(qei->chZ.ddr, qei->chZ.mask);
}

static inline void AJ_QEI_Z_ConfigAsPullNone(aj_qei_t *qei){
    AJ_BitReg_ClearBit_Mask(qei->chZ.port, qei->chZ.mask);
}

static inline void AJ_QEI_Z_ConfigAsPullUp(aj_qei_t *qei){
